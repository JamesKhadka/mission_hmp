          ...
          {qualifications?.map((item, index) => (
            <li key={index} className='flex flex-col sm:flex-row sm:justify-between sm:items-end md:gap-5 mb-[30px]'>
              <div>
                ...
                <p className='text-[16px] leading-5 font-medium text-textColor'>{item.degree}</p>
              </div>
        ...
        <ul className='grid sm:grid-cols-2 gap-[30px] pt-4 md:p-5'>
          ...
          {experiences?.map((item, index) => (
            <li key={index} className='p-4 rounded bg-[#fff9ea]'>
              <span className='text-irisBlueColor text-[15px] leading-6 fony-semibold'>{FormateDate(item.startingDate)} - {FormateDate(item.endingDate)}</span>
